...
export function CellFrequencyModal({ isOpen, onClose, cell, churchId }) {
    ...
    return (
        <>
            <Modal isOpen={isOpen} onClose={onClose} title="Frequência da Célula" className="sm:max-w-xl">
                <div className="space-y-8 max-h-[70vh] overflow-y-scroll p-1 pr-2">
                    ...
                    <div className="space-y-6">
                        {/* Header: Only show "Nova Célula" button if NOT adding */}
                        {!isAddingSupport ? (
                            <>
                            ...
                            // Add Form View (Replaces List)
                            <div className="animate-in fade-in slide-in-from-right-4">
                                <div className="flex items-center justify-between pb-2 border-b border-orange-100 pt-2 mb-4">
                                    <div className="flex items-center gap-2">
                                        <Users className="w-5 h-5 text-orange-600" />
                                        <h3 className="text-lg font-bold text-slate-800">Nova Célula de Apoio</h3>
                                    </div>
                                    <button
                                ...
                                <form onSubmit={handleCreateSupportCell} className="bg-orange-50 p-6 rounded-lg border border-orange-200">
                                    <div className="space-y-4 mb-6">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full rounded-md border-orange-200 focus:border-orange-500 focus:ring-orange-500"
                                                required
                                                autoFocus
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 mb-1">Líder (Opcional)</label>
                                            <input
                                                type="text"
                                    ...
                                    <div className="flex justify-end gap-3">
                                        <button
                                            type="button"
                                            onClick={() => setIsAddingSupport(false)}
                                            className="px-4 py-2 rounded-md text-slate-600 hover:text-slate-800 text-sm font-medium"
                                        >
                                            Cancelar
                                        </button>
                                        <button type="submit" className="bg-orange-600 text-white px-6 py-2 rounded-md hover:bg-orange-700 text-sm font-medium shadow-sm transition-colors">
                                            Salvar Célula
                                        </button>
                                    </div>
                                </form>
            ...
            {/* Add Support Cell Modal */}
            <Modal
                isOpen={isAddingSupport}
                onClose={() => setIsAddingSupport(false)}
                title="Cadastrar Célula de Apoio"
                className="sm:max-w-md"
            >
                <form onSubmit={handleCreateSupportCell} className="space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Nome da Célula</label>
                        <input
                            type="text"
                            value={newSupportName}
                            onChange={(e) => setNewSupportName(e.target.value)}
                            placeholder="Ex: Célula Betel"
                            className="w-full rounded-md border-slate-300 focus:border-orange-500 focus:ring-orange-500"
                            required
                            autoFocus
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Líder (Opcional)</label>
                        <input
                            type="text"
                            value={newSupportLeader}
                            onChange={(e) => setNewSupportLeader(e.target.value)}
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button
                            type="button"
                            onClick={() => setIsAddingSupport(false)}
                            className="px-4 py-2 rounded-md text-slate-600 hover:text-slate-800 text-sm font-medium border border-slate-300 hover:bg-slate-50 transition-colors"
                        >
                            Cancelar
                        </button>
                        <button
                            type="submit"
                            className="bg-orange-600 text-white px-6 py-2 rounded-md hover:bg-orange-700 text-sm font-medium shadow-sm transition-colors"
                        >
                            Salvar Célula
                        </button>
                    </div>
