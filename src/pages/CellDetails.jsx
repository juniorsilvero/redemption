        ...
        <div className="space-y-6">
                    ...
                    <button
                        onClick={() => {
                            ...
                            navigator.clipboard.writeText(link)
                                .then(() => toast.success('Link da ficha copiado! Compartilhe com os passantes.'))
                                .catch(() => toast.error('Não foi possível copiar o link.'));
                        }}
                        className="flex items-center justify-center gap-2 rounded-md bg-slate-700 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-600 w-full sm:w-auto"
                    >
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                ...
                <Card className="border-0 shadow-sm ring-1 ring-slate-100 bg-white">
                    <CardHeader 
                        className="border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between min-h-[64px] py-3 cursor-pointer select-none"
                        onClick={() => setIsWorkersOpen(!isWorkersOpen)}
                    >
                        <div className="flex items-center gap-2">
                            {isWorkersOpen ? <ChevronDown className="h-4 w-4 text-slate-400" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
                            <CardTitle className="text-sm font-semibold text-slate-900">
                                Trabalhadores 
                                <span className="ml-2 inline-flex items-center rounded-full bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-600/10">
                                    {workers?.length || 0}
                                </span>
                            </CardTitle>
                        </div>
                ...
                <Card className="border-0 shadow-sm ring-1 ring-slate-100 bg-white">
                    <CardHeader 
                        className="border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between min-h-[64px] py-3 cursor-pointer select-none"
                        onClick={() => setIsPassersOpen(!isPassersOpen)}
                    >
                        <div className="flex items-center gap-2">
                            {isPassersOpen ? <ChevronDown className="h-4 w-4 text-slate-400" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
                            <CardTitle className="text-sm font-semibold text-slate-900">
                                Passantes 
                                <span className="ml-2 inline-flex items-center rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 ring-1 ring-inset ring-emerald-600/10">
                                    {passers?.length || 0}
                                </span>
                            </CardTitle>
                        </div>
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500">
                                                    ...
                                                    {person.age && <span className="ml-2 bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 font-medium">{person.age} anos</span>}
                                                </p>
                                            </div>
                                        </div>
